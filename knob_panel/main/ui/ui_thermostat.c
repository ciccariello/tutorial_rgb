...
static void thermostat_event_cb(lv_event_t *e)
{
    ...
    if (LV_EVENT_FOCUSED == code) {
        ...
        if (is_time_out(&time_500ms)) {
            ...
            lv_roller_set_selected(temp_wheel, (current - 8), LV_ANIM_ON);
        }
...
void ui_thermostat_init(lv_obj_t *parent)
{
    ...
    lv_arc_set_value(temp_arc, (param->hour) );
    lv_arc_set_range(temp_arc, 8, 19);
    lv_obj_set_style_arc_width(temp_arc, 10, LV_PART_MAIN);
    lv_obj_set_style_arc_width(temp_arc, 10, LV_PART_INDICATOR);

    lv_obj_set_style_arc_color(temp_arc, lv_color_make(36, 163, 235), LV_PART_MAIN);
    lv_obj_set_style_arc_color(temp_arc, lv_color_make(36, 163, 235), LV_PART_INDICATOR);
...
//    lv_obj_align(img_temp_unit, LV_ALIGN_CENTER, 50, -10);
    ...
    lv_roller_set_selected(temp_wheel, ( (param->hour) - 8 ), LV_ANIM_ON);
    ...
    lv_anim_set_time(&a1, 400 * 5);
    lv_anim_start(&a1);

    lv_anim_t a2;
    lv_anim_init(&a2);
    lv_anim_set_var(&a2, img_thermostat_temp);
    lv_anim_set_values(&a2, lv_obj_get_y_aligned(img_thermostat_temp) - 20, lv_obj_get_y_aligned(temp_arc) + 13 );
...
//    lv_obj_add_event_cb(page, thermostat_event_cb, LV_EVENT_LONG_PRESSED, NULL);
    lv_obj_add_event_cb(page, thermostat_event_cb, LV_EVENT_CLICKED, NULL);
